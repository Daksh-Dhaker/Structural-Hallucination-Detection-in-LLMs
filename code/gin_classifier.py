...
class GINGraphClassifier(torch.nn.Module):
    def __init__(self, num_node_features, hidden_channels, num_classes, graph_feat_dim=0, dropout=0.3):
        super(GINGraphClassifier, self).__init__()

        self.graph_feat_dim = graph_feat_dim

        nn1 = Sequential(Linear(num_node_features, hidden_channels), ReLU(), BatchNorm1d(hidden_channels))
        nn2 = Sequential(Linear(hidden_channels, hidden_channels), ReLU(), BatchNorm1d(hidden_channels))
        nn3 = Sequential(Linear(hidden_channels, hidden_channels), ReLU(), BatchNorm1d(hidden_channels))

        self.conv1 = GINConv(nn1)
        self.conv2 = GINConv(nn2)
        self.conv3 = GINConv(nn3)

        # classifier input dim = hidden_channels + graph_feat_dim
        self.fc = Sequential(
            Linear(hidden_channels + graph_feat_dim, hidden_channels // 2),
            ReLU(),
            Dropout(dropout),
    ...
    def forward(self, data):
        ...
        # append subgraph features if available
        if hasattr(data, "subgraph_features"):
            x = torch.cat([x, data.subgraph_features], dim=1)
...
def train(model, train_loader, optimizer, criterion, device):
    ...
    for data in train_loader:
        data = data.to(device)
        out = model(data)  # ✅ FIXED: pass only the whole data object
...
if __name__ == '__main__':
    ...
    model = GINGraphClassifier(num_node_features=num_features, hidden_channels=64,
                           num_classes=2, graph_feat_dim=0)
