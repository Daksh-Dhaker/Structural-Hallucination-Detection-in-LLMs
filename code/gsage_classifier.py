...
from torch_geometric.nn import GCNConv, global_mean_pool, GATConv, GlobalAttention, SAGEConv
...
class GraphSAGEGraphClassifier(torch.nn.Module):
    def __init__(self, num_node_features, hidden_channels, num_classes, graph_feat_dim=0, dropout=0.3):
        super(GraphSAGEGraphClassifier, self).__init__()
    ...
    def forward(self, data):
        ...
        # --- Optionally concatenate mined subgraph features ---
        if hasattr(data, "subgraph_feats"):
            x = torch.cat([x, data.subgraph_feats.to(x.device)], dim=1)
...
if __name__ == '__main__':
    ...
    model = GraphSAGEGraphClassifier(num_node_features=num_features, hidden_channels=64,
