...
from models.edge_attribute_patcher import EdgeAttributePatcher
    ...
    for i in range(len(layer_nodes)):
        for key in layer_nodes[i]:
            layer_nodes[i][key] = sorted(
                layer_nodes[i][key], key=lambda x: int(x[0].split(".")[4])
            )
    return layer_nodes
    ...
    for i in range(len(layer_nodes)):
        attn_head_imp = torch.stack(
            [
                ...
                torch.stack([n[1] for n in layer_nodes[i]["k_proj"]]).repeat_interleave(
                    4, dim=-1
                ),
...
def combine_circuits(circuits: dict[str, nx.DiGraph], max_layers: int):
    ...
    first_key = next(iter(circuits))
    combined_graph = circuits[first_key].copy()
...
def visualize_circuit2(nx_graph: nx.DiGraph, out_file: str, max_layers: int):
    label_map = {
        "input": "input",
        "output": "output",
        "attn_head": "A",
    ...
    # Draw each type separately (so we can vary color/shape)
    for node_type, color in color_map.items():
        nodes_of_type = [n for n in nx_graph.nodes if n.split(",")[-1] == node_type]
