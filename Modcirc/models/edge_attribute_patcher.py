...
class EdgeAttributePatcher:
    ...
    def _activation_hook(self, name: str):
        ...
        def hook(module, input, output):
            self.activation_cache[name] = {
                "input": [
                    x.detach() if isinstance(x, torch.Tensor) else x for x in input
                ],
    ...
    def get_activation(self, x_clean: str, x_corrupted: str) -> torch.Tensor:
        ...
        prompt = self.tokenizer.apply_chat_template(
            [
                ...
                {"role": "user", "content": x_clean},
            ],
            add_generation_prompt=True,
            tokenize=False,
        )
        inputs_clean = self.tokenizer(prompt, return_tensors="pt").to(self.device)
        ...
        prompt = self.tokenizer.apply_chat_template(
            [
                ...
                {"role": "user", "content": x_corrupted},
            ],
            add_generation_prompt=True,
            tokenize=False,
        )
        inputs_corrupted = self.tokenizer(prompt, return_tensors="pt").to(self.device)
    ...
    def get_gradient(self, clean_outputs, ground_truth):
        ...
        for name in self.get_all_comp_nodes():
            ...
            if hasattr(module, "weight") and module.weight.grad is not None:
                self.gradient_cache[name] = module.weight.grad.detach()
    ...
    def get_causal_effect(self, comp_node: str) -> tuple[float | list[float], torch.Tensor | list[torch.Tensor]]:
        ...
        if comp_node.split(".")[-1] in ['q_proj', 'v_proj', 'k_proj']:
            activation_diff = activation_diff.reshape(-1,1,self.model.config.head_dim)
            gradient = gradient.reshape(-1, self.model.config.head_dim, gradient.shape[-1])
            clean_acts = clean_activation.reshape(-1, self.model.config.head_dim).cpu()            
            res = torch.bmm(activation_diff, gradient).mean(-1).squeeze().cpu()
        else:
            activation_diff = activation_diff.reshape(-1,1,1)
            gradient = gradient.unsqueeze(1)
            clean_acts = clean_activation.reshape(-1, 1).cpu()
            res = (activation_diff * gradient).mean(-1).squeeze().cpu()
